...
$(document).ready(function(){
	checkButton();
	loadDescription();
	player2mp3("player2mp3","/ZDesk/zh_cn/ZQC/hongshanguo.mp3");
});
	...
	function  loadDescription(){
		...
		if(num!=0){
			...
			for(var i=0;i<commonTreeParam.length;i++){
								   ...
								   if(k<4){
									   //各项评分
									   $("#tr_"+i+"").append("<td style='text-align:center;' bgcolor='#FFFFCC'>"+array[k].DESCRIPTION+"</td>");
									   //最大值最小值
									   arraymin.push(array[k].min_value);
									   arraymax.push(array[k].max_value);
									   $("#tr_tr_"+i+"").append("<td style='text-align:center;' bgcolor='#FFFFCC'>"+array[k].min_value+'~'+array[k].max_value+"</td>");
									   //分值
									   var t=array[k].value_remark;
						               aa[i+'_'+k]=t;
						               var q=i+"_"+k;
						               var p='pfx_00'+sum;
									   $("#tr_tr_tr_"+i+"").append("<td bgcolor='#FFFFCC'><input type='text' id='pfx_00"+sum+"' value='"+score[0][p]+"' class='input' name='pfx_00"+sum+"' style='padding: 0; margin: 0;  width: 100%; text-align:center;' readonly='readonly'  onchange=g("+array[k].min_value+","+array[k].max_value+",'"+p+"') onclick=f('"+q+"') onfocus=f('"+q+"')></td>");
								   ...
								   if(k>=4){
								   		if(k%4==0){
								   		  ...
								   		  //各项评分
									   $("#genjiedian_"+i+" #tr_k"+row+"").append("<td style='text-align:center;' bgcolor='#FFFFCC'>"+array[k].DESCRIPTION+"</td>");
									   //最大值最小值
									   arraymin.push(array[k].min_value);
									   arraymax.push(array[k].max_value);
									   $("#genjiedian_"+i+" #tr_tr_k"+row+"").append("<td style='text-align:center;' bgcolor='#FFFFCC'>"+array[k].min_value+'~'+array[k].max_value+"</td>");
									   //分值
									   var t=array[k].value_remark;
						               aa[i+'_'+k]=t;
						               var q=i+"_"+k;
						               var p='pfx_00'+sum;
									   $("#genjiedian_"+i+" #tr_tr_tr_k"+row+"").append("<td bgcolor='#FFFFCC'><input type='text' id='pfx_00"+sum+"' value='"+score[0][p]+"' class='input' name='pfx_00"+sum+"' style='padding: 0; margin: 0;  width: 100%; text-align:center;' readonly='readonly'  onchange=g("+array[k].min_value+","+array[k].max_value+",'"+p+"') onclick=f('"+q+"') onfocus=f('"+q+"')></td>");
