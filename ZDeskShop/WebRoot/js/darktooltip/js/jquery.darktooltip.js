...
(function($) {
	...
	DarkTooltip.prototype = {
		...
		setPositions: function(){
			var x_number = this.options.x_number;
			var y_number = this.options.y_number;
			if(!(/^[0-9]*$/.test(x_number))||x_number==undefined){
				x_number=0;
			}
			if(!(/^[0-9]*$/.test(y_number))||y_number==undefined){
				y_number=0;
			}
			var leftPos = 0;
			var topPos = 0;
			var bearerTop = this.bearer.offset().top-y_number;
			var bearerLeft = this.bearer.offset().left-x_number;
		...
		setEvents: function(){
			...
			if(this.options.show == true&&this.options.show!=undefined){
				dt.setPositions();
				dt.show();
			}else if(this.options.show == false&&this.options.show!=undefined){
				dt.hide();
			}
		},
