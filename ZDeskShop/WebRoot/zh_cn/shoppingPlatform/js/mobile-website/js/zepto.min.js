...
var Zepto = function () {
	...
	return A.matches = function (a, b) {
		...
		var b = F(a);
		if (this[0] && !b) {
			var d = c(a).get(0), e = d.parentNode || this.length > 1;
		}
...
window.Zepto = Zepto, "$" in window || (window.$ = Zepto), function (a) {
		...
		return this.each(function (e, f) {
			n(f, c, d, b, function (c) {
				return function (d) {
					...
					if (g) {
						return e = a.extend(t(d), {currentTarget:g, liveFired:f}), c.apply(g, [e].concat([].slice.call(arguments, 1)));
					}
				};
			});
		});
	...
	a.each(g, function (a, e) {
		...
		return a.isPlainObject(c) && (d = c.easing, e = c.complete, c = c.duration), c && (c = (typeof c == "number" ? c : a.fx.speeds[c] || a.fx.speeds._default) / 1000), this.anim(b, c, d, e);
	}, a.fn.anim = function (c, d, e, f) {
		var g, h = {}, i, t = "", u = this, v, w = a.fx.transitionEnd;
		d === b && (d = 0.4), a.fx.off && (d = 0);
		if (typeof c == "string") {
			h[o] = c, h[p] = d + "s", h[q] = e || "linear", w = a.fx.animationEnd;
		} else {
			i = [];
			for (g in c) {
