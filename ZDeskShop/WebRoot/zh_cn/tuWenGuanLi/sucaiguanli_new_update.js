...
//选择列别对应的内容分类
function selectContentClass(Category,flag){
...
function tianjiazituwenxinxi_(){
		...
		$("#button_tianjiazitu").before('<div id="Img_Div'+zituwengeshu+'" style="margin-bottom: 5px;width: 380px; height: 100px;  border:1px solid #F5F5F5; border-radius:12px;background-color: #F5F5F5;margin-left:15px;margin-right:0px  ">'+
    ...
    function baocun(){
	    ...
	    var allPic=[];
	     ...
	     var type="";
	               ...
	               //alert("baocun:"+JSON.stringify(jsonshuju));
	     for(var key in jsonshuju){
	         if(jsonshuju[key].parentId=='0'&&jsonshuju[key].isRelease=="未发布"){
	            	key1=key;
	                type=jsonshuju[key].Category;
		            jsonshuju[key].releaseTime="0000-00-00 00:00:00";
		            jsonshuju[key].releasePeople="";
		            //获取图片
		           // jsonshuju[key].coverUrl=$("#img_Article_Img"+jsonshuju[key].id)[0].src;
		            if(jsonshuju[key].coverUrl == "" || jsonshuju[key].coverUrl == null 
		              			|| jsonshuju[key].coverUrl =="undefined"){
		              	 var str = key.substr(9,key.length);		
						 jsonshuju[key].coverUrl=$("#img_Article_Img"+str)[0].src;	
		            }else{
			            jsonshuju[key].coverUrl=$("#img_Article_Img"+jsonshuju[key].id)[0].src;
		            }
		            jsonshuju[key].singleOrDouble="多图文";
		            if(jsonshuju[key].originalLink==""||jsonshuju[key].originalLink==null){
		               jsonshuju[key].originalLink="/ZingMH/zh_cn/tuWenGuanLi/graphicText.html?id="+jsonshuju[key].id;
		            }
	       ...
	       for(var key in jsonshuju){
	            if(jsonshuju[key1].isRelease=="已发布"&&key1!=key){
		              ...
		              //获取图片
		              //如果这个coverUrl是空，则把这个tablefrom 截取就来获取图片
		              if(jsonshuju[key].coverUrl == "" || jsonshuju[key].coverUrl == null 
		              			|| jsonshuju[key].coverUrl =="undefined"){
		              	 var str = key.substr(9,key.length);		
						 jsonshuju[key].coverUrl=$("#img_Article_Img"+str)[0].src;	
	            ...
	            if(jsonshuju[key1].isRelease=="未发布"&&key1!=key){
	               	   t=1;
		               jsonshuju[key].releaseTime="0000-00-00 00:00:00";
		               jsonshuju[key].Category=type;
		               jsonshuju[key].isRelease="未发布";
		               //获取图片
		               //jsonshuju[key].coverUrl=$("#img_Article_Img"+jsonshuju[key].id)[0].src;
		               if(jsonshuju[key].coverUrl == "" || jsonshuju[key].coverUrl == null 
		              			|| jsonshuju[key].coverUrl =="undefined"){
	       ...
	       if(t==0){
	          if(jsonshuju[key1].originalLink==""||jsonshuju[key1].originalLink==null){
		             jsonshuju[key1].originalLink="/ZingMH/zh_cn/tuWenGuanLi/graphicText.html?id="+jsonshuju[key1].id;
		       }
...
function xiugai(xiugaiId){
	...
	function xiebiaodan1(){
		...
		$("#img_Article_Img"+xiugaitupianId+"").attr("src", json.coverUrl); 
		if(json.textContent==undefined||json.textContent==""){
			UE.getEditor('editor').setContent(" ",false);
		}else{
			UE.getEditor('editor').setContent(json.textContent,false);
...
function shanchu(a){
	   ...
	   //主键默认为id  如不为id 加上params.primarykey
			    ...
			    if(tupianjilu.hasOwnProperty("Img_Div"+a+"")&&(tupianjilu["Img_Div"+a+""]!=null||tupianjilu["Img_Div"+a+""]!="")){
			      ...
			      alert(shagngejiedian);
			      if(shagngejiedian.substr(shagngejiedian.indexOf("Img_Div")+7, shagngejiedian.length)==yincangsuoyongId){
			      xianshi();
