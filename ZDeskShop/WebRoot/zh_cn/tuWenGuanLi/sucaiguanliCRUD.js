...
function getGroup(){
	...
	commonSelect(params,false,function(data){
		if(data && data.success){
			$("#Category1").empty();
			$("#Category1").append("<option value=''></option>");
			for(var o in data.data){
				$("#Category1").append("<option value='"+data.data[o].categoryValue+"'>"+data.data[o].categoryValue+"</option>");
			}
		}else{
			alert(data.mgs);
		}			
