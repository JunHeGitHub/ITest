...
function initData() {
	...
	*/
    ...
    pat.nameSpaceId = 'wenBenZuoYeFuJianPingFen_initData';
...
function doSeach(data, callback, settings){
    var params={};
    var rdata={};
    params.tableName="SU_QC_SOURCE_RECORD_DATA";
    params.offset=settings._iDisplayStart;
    params.rows=settings._iDisplayLength;
    params.data_state_parames="'"+zinglabs.zqc.getState("fujianyifenpei")+"','"+zinglabs.zqc.getState("fujianweifenpei")+"','"+zinglabs.zqc.getState("chujianyifabu")+"'";
    params.score_state_parames="'"+zinglabs.zqc.getState("chujianyipingfen")+"','"+zinglabs.zqc.getState("fujianyipingfen")+"','"+zinglabs.zqc.getState("fujianweipingfen")+"'";
   	if(isFirstLoad){
		if(searchItem2sql("searchDataForm")){
				params.searchItems=searchItem2sql("searchDataForm")
			
		}else{
			isFirstLoad=false;
		}
	}
    var url = "/"+PRJ_PATH+"/"+ZQC_ROU+"/ZQC/queryWenBenFenPei.action";
     ...
     function(data) {
              if(data&&data.data){
              	...
              	////总条数
              	rdata.recordsTotal=getTotalRow(params);
              	//过来没有使用到总条数
              	rdata.recordsFiltered=getTotalRow(params);
              	//代表第几次渲染页面
              	rdata.draw=settings.iDraw;
              	callback(rdata);
              }else{
...
//获取数据总条数
function getTotalRow(params){
	var total=0;
	var url = "/"+PRJ_PATH+"/"+ZQC_ROU+"/ZQC/getTotalRow.action";
...
function searchData2() {
	isFirstLoad=true;
	var tableTmp = $("#tableForData").DataTable();
	tableTmp.draw();
}
...
function searchData(){
	...
	var pat={};	
	pat.nameSpace = 'com.zinglabs.apps.mybaits_xml_def.ZQC';
    pat.nameSpaceId = 'wenBenZuoYeFuJianPingFen_search';
...
function pf(dt){
	...
	var params={
		title:"文本作业评分",
		url:"/"+window.top.PRJ_PATH+"/zh_cn/ZQC/wenbenzuoyepingfen.html?data_source=fujian&id="+dt.id+"&score_state="+dt.score_state+"&closeFn="+searchData+"&grade_name="+dt.grade_name+"&file_name="+dt.file_name+"&file_location="+dt.file_location,
		width:"900"
	};
...
function quXiaoFenPei(){
	...
	if(selectData && selectData!=null && selectData!=undefined){
		...
		if(c != 0){
			//alert("所要取消分配的数据中存在文本状态初检已发布、初检已评分或复检未分配的数据！");
...
function choseZhiJianYuan(){
	var params={
		title:"质检员列表",
		content: '<iframe src="/'+window.top.PRJ_PATH+'/zh_cn/ZQC/zhijianyuanliebiao.html" style="border: 0; width: 100%; height: 100%"  frameborder="0"/>',
		width:"900",
		height:"570",
		ok: function(){
