...
function protExecl(){
	...
	for(var i=0;i<selectData.length;i++){
		if(selectData[i].score_state==zinglabs.zqc.getState("fujianweipingfen")){
			shifouweipingfen++;	
			break;
		}
	...
	//}
		var excelAttr={};
		excelAttr.fileName="复检评分详情"+shijianchuo()+".xls";
		excelAttr.fTitle="复检评分详情";
		...
		excelAttr.pingFenTableName='T_GRADE_DETAIL_FUJIAN_NEW';
		excelAttr.pingYuTableName='fujian_remark';
...
function initData() {
	...
	*/
    ...
    pat.nameSpaceId = 'yuYinZhiJianYuanFuJianPingFen_initData';
...
function searchData(){	
	...
	var pat={};	
	pat.nameSpace = 'com.zinglabs.apps.mybaits_xml_def.ZQC';
    pat.nameSpaceId = 'yuYinZhiJianYuanFuJianPingFen_search';
...
function pf(dt){
	var params={
		title:"语音作业评分",
		url:"/"+window.top.PRJ_PATH+"/zh_cn/ZQC/RS_yuyinzuoyepingfen.html?data_source=fujian&id="+dt.id+"&closeFn="+searchData+"&score_state="+dt.score_state+"&grade_name="+dt.grade_name+"&file_name="+dt.file_name+"&file_location="+dt.file_location,
		width:"900"
	};
