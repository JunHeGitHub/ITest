	...
	//通用查询@@		
			function doSearch(){
				   ...
				   var pat = zkm_getHtmlFormJsons('searchDataForm');
					...
					//根据分机号查询
					pat.agent_user=getUserInfo().phone_number;
...
//查看查看详情评�
	...
	function d(e){
			...
			if(getfile_Path(file).length!=0){
				var file_location=getfile_Path(file)[0].file_location;
			}
		    ...
		    title: '查看文本评分',
		     width:'900',
		      height:'600',
		    content: '<iframe src='+e+'?grade_id='+grade_id+'&grade_name='+grade_name+'&file_location='+file_location+'&file_name='+file+'&score_state='+score_state+'&checkBtn='+checkBtn+' style="border: 0; width: 100%; height: 100%"  frameborder="0"/>'
...
//获取下载路径
function getfile_Path(file_name){
	var rdata;
		var pat = {};
		pat.tableName = "T_GRADE_DETAIL_FUSHEN_NEW";
		pat.primaryKey = "id";
...
function chaKanWenBenNeiRong(){
	...
	if(fnGetSelected().length!=0){
		if(getfile_Path().length!=0){
			var file_location=getfile_Path()[0].file_location;
		}
...
function xianShiPingYu(pingyu,str){	
    if(str=='agent_user_remark'){
    	$("#pingyu_div").html("<b>座席复审说明</b><br/>"+pingyu);
    }
    if(str=='grade_remark'){
    	$("#pingyu_div").html("<b>评分评语</b><br/>"+pingyu);
    }
    if(str=='admin_remark'){
    	$("#pingyu_div").html("<b>质检复审说明</b><br/>"+pingyu);
    }
    $("#pingyu_div").slideDown(200);
}

function pingyu_div_hide(){
	$("#pingyu_div").slideUp(0);
}
