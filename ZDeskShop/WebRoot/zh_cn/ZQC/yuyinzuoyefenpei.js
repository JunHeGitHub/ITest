...
function initData() {
	...
	*/
	...
	pat.nameSpaceId = "yuYinZuoYeFenPei_initData";
...
function quXiaoLingQu() {
	...
	if (selectData && selectData != null && selectData != undefined) {
		...
		if (c != 0) {
			//alert("所要取消领取的数据中存在语音状态初检未分配和0的数据！");
...
function searchData() {
	...
	var pat = {};
	pat.nameSpace = "com.zinglabs.apps.mybaits_xml_def.ZQC";
	pat.nameSpaceId = "yuYinZuoYeFenPei_search";
...
//function searchData(data, callback, settings){
//                var params=spellSelectParams("wenbenData");
//			    params.tableName="SU_QC_SOURCE_RECORD_DATA";
//			    params.beanId="commonFreeFilter";
//         		params.nameSpace="com.zinglabs.apps.mybaits_xml_def.ZQC.yuYinZuoYeFenPei_search";
//         		params.data_state_parames = "'" + zinglabs.zqc.getState("zero") + "','" + zinglabs.zqc.getState("chujianweifenpei") + "','" + zinglabs.zqc.getState("chujianyifenpei") + "'";
//				params.score_state_parames = "'" + zinglabs.zqc.getState("zero") + "','" + zinglabs.zqc.getState("chujianweipingfen") + "'";
...
function choseZhiJianYuan() {
	var params = {title:"\u8d28\u68c0\u5458\u5217\u8868", content: '<iframe src="/'+window.top.PRJ_PATH+'/zh_cn/ZQC/zhijianyuanliebiao.html" style="border: 0; width: 100%; height: 100%"  frameborder="0"/>', width:"900", height:"570", ok:function () {
...
//调听录音
function listening(){
	var fnGetSe=fnGetSelected();
	if(fnGetSelected().length==0){
		zinglabs.i18n.alert("ZJGL_xuanzeshuju");
		return;
	}
	if(fnGetSelected().length>1){
		zinglabs.i18n.alert("ZQC_zhinengdiaotingyitiaoluyin");
		return;
	}
	if(fnGetSelected().length==1){
		var file_name=fnGetSelected()[0].file_name;
	}
	
	var params={
		title:"语音内容",
		url:"/"+window.top.PRJ_PATH+"/zh_cn/ZQC/lookyuyinneirong.html?file_name="+file_name
	};
