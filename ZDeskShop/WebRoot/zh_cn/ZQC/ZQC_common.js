			...
			if(e.id.length>5){				
				if(e.id.substring(e.id.lastIndexOf("_")+1,e.id.length-1)=="DFT"){
					...
					if(e.value!=""){
						...
						//判断评分输入框
						if(e.id.substring(0,e.id.length-4)=="pfx_total"){
							if(e.id.substring(e.id.lastIndexOf("_")+1,e.id.length)=="min"){
								searchItems+=" "+e.id.substring(0,e.id.length-4)+" >= '"+e.value+"' and ";
							}
							if(e.id.substring(e.id.lastIndexOf("_")+1,e.id.length)=="max"){
								searchItems+=" "+e.id.substring(0,e.id.length-4)+" <= '"+e.value+"' and ";
							}
						}else{
...
function initPingFenZhuangTai(source,id){
	...
	if(source=="wenbenpingfenchaxun"){
		$("#"+id).append("<option value="+zinglabs.zqc.getState("chujianyipingfen")+">"+zinglabs.zqc.getState("chujianyifabu")+"</option>");
		$("#"+id).append("<option value="+zinglabs.zqc.getState("zuoxishangsu")+">"+zinglabs.zqc.getState("zuoxishangsu")+"</option>");
		$("#"+id).append("<option value="+zinglabs.zqc.getState("zuoxishangsuchenggong")+">"+zinglabs.zqc.getState("zuoxishangsuchenggong")+"</option>");
		$("#"+id).append("<option value="+zinglabs.zqc.getState("zuoxishangsushibai")+">"+zinglabs.zqc.getState("zuoxishangsushibai")+"</option>");
	}
...
function xiaZaiWenBen(){
	...
	var file_url;
	var is_mp3_html=file_name.substring(file_name.lastIndexOf("."),file_name.length);
	var folder=file_name.substring((file_name.lastIndexOf("_")+1),(file_name.lastIndexOf("_")+9));
	var file_url='';
	if(is_mp3_html=='.mp3'){
		file_url="/usr/local/nginx-1.7.2/html/"+window.top.PRJ_PATH+"/ZIM/record_data/"+folder+"/"+file_name;
		//file_url="/usr/local/nginx-1.7.2/html/ZDesk/zh_cn/ZQC/9.253.16.10/record_data/"+folder+"/"+file_name;
		//file_url="9.253.16.10/record_data/"+folder+"/"+file_name;	
		//file_url="c:/hongshanguo.mp3";	
	}
	if(is_mp3_html=='.html'){
		 file_url="/usr/local/nginx-1.7.2/html/"+window.top.PRJ_PATH+"/ZIM/record_data/"+folder+"/"+file_name;
		//file_url="/usr/local/nginx-1.7.2/html/ZDesk/zh_cn/ZQC/9.253.16.10/record_data/"+file_name.substring(file_name.lastIndexOf("_")+1,file_name.lastIndexOf("_")+9)+"/"+file_name;	
	}
	commonDownload(file_url,""+file_name+"");
}

//百分数转换为小数
function parseDecimals(num){
...
function shijianchuo(){
	...
	var ss=myDate.getSeconds()<10?'0'+myDate.getSeconds():myDate.getSeconds();
	var time=hh+''+mm+''+ss;
	var sysdate=""+date+""+time+""+myDate.getTime();
	return sysdate;
}
