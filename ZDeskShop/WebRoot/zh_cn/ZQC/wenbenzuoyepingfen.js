	...
	pat.nameSpace = 'com.zinglabs.apps.mybaits_xml_def.ZQC';
	  ...
	  return rdata;	
	    ...
	    return rdata;	
			...
			function chujianbaocun(){
			  ...
			  var cfs=$("#dongtaichuangjianTable select[name]");
			   ...
			   for(var i=0;i<cfs.length;i++){
				...
				commonCRUDForComplex(newPat, false,//执行删除
		    	function(data) {
		    	}); 
				...
				//alert(JSON.stringify(newPat));
				commonCRUDForComplex(newPat, false,//执行插入
		    	function(data) {
		    	            var pat={};
			...
			getCanshu['grade_remark_fj']=$("#fujian_remark").val();
			 getCanshu['grade_remark']=$("#fushen_remark").val();
			  var zong=0;
			  var fenshu={};
			  var cfs=$("#dongtaichuangjianTable select[name]");
				for(var i=0;i<cfs.length;i++){
...
function Update_shangsu(){
		...
		pat.agent_user_remark=$("#grade_remark").val();
		pat.admin_remark=$("#fushen_remark").val();
		pat.grade_remark=$("#shangsu_remark").val();
