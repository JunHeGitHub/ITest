$(document).ready(function(){
		initPingFenZhuangTai("wenbenpingfenchaxun","score_state");
...
function timeCheck(){
    ...
    var gbt=$("#generate_time_state_DFTB").val();
    var get=$("#generate_time_state_DFTE").val();
    gbt=new Date(Date.parse(gbt.replace(/-/g,"/")));
	get=new Date(Date.parse(get.replace(/-/g,"/")));
    ...
    if(et.getTime()<bt.getTime()||get.getTime()<gbt.getTime()){
		zinglabs.i18n.alert('ZQC_jieshushijianbunengdayukaishihijian');
		return false;
	}
   if((U_DateDiffer(bt,et)/24)>30||(U_DateDiffer(gbt,get)/24)>30){
