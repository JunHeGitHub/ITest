	...
	//通用查询@@		
			function doSearch(){
				   ...
				   var pat = zkm_getHtmlFormJsons('searchDataForm');
					...
					pat.nameSpaceId="shujuchaxunyuyinpingfenchaxun_select";
					pat.is_mp3='mp3';
					pat.ling='0';
		...
		for(var i=0;i<rdata.length;i++){
			var gradeId=rdata[i].file_name;
			rdata[i].grade_id+=gradeId.substring(0,gradeId.lastIndexOf("."));
		}
		return rdata;
	}
		...
		zinglabs.i18n.alert("zuoxichaxun_qingxuanzeyaochakandeshuju");
			...
			//按评分状态筛选
				function tableSearch(){
						$("#updateFormDiv").hide();	
						var begin=$("#begin_time_devoir_DFTB").val();
						var end=$("#begin_time_devoir_DFTE").val();
						if(begin==''||begin==null){
...
function hideTooltip(id){
	...
	function d(e){
		    ...
		    content: '<iframe src='+e+'?grade_id='+grade_id+'&agent_user='+agent_user+'&closeFn='+tableSearch1+'&grade_name='+grade_name+'&checkBtn='+checkBtn+'&data_state='+data_state+'&score_state='+score_state+'&id='+id+'&total='+total+'&owner='+owner+'&file_name='+file_name+'&generate_time_state_DFTB='+generate_time_state_DFTB+'&score_state='+score_state+' style="border: 0; width:100%; height:100%" id="iframe1"  frameborder="0"/>'
	...
	//判定是否选中数据，查看各项评分
		function selectData(){
			...
			if(fnGetSelected().length==0){
				 zinglabs.i18n.alert("pingfen_qingxuanzeyaochakandeshuju");
			}else{
