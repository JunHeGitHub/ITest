...
function initForm(data){
	...
	function userGroup(){
					   ...
					   var pat ={};
						...
						pat.nameSpaceId="SU_QC_SOURCE_RECORD_DATA_select_wulizu";
						//当前登录用户
			...
			function doSearch(){
					   ...
					   var pat = zkm_getHtmlFormJsons('searchDataForm');
						...
						//当前登录用户
						var wulizu=userGroup();
						if(wulizu.length!=0){
							pat.alias2=userGroup()[0].alias2;
							pat.is_mp3='txt';
...
function timeCheck(){
    ...
    var bt2=$("#generate_time_state").val();
    var et2=$("#generate_time_state_end").val();
...
function hideTooltip(id){
	...
	function d(e){
		    ...
		    title: '查看文本评分',
		     width:'900',
		      height:'600',
		    content: '<iframe src='+e+'?grade_id='+grade_id+'&agent_user='+agent_user+'&closeFn='+tableSearch+'&grade_name='+grade_name+'&file_location='+file_location+'&data_state='+data_state+'&score_state='+score_state+'&id='+id+'&total='+total+'&owner='+owner+'&file_name='+file_name+'&generate_time_state='+generate_time_state_DFTB+'&score_state='+score_state+' style="border: 0; width: 100%; height: 100%"  frameborder="0"/>'
	...
	//判定是否选中数据，查看各项评分
		function selectData(){
			...
			if(fnGetSelected().length==0){
				...
				d("/"+window.top.PRJ_PATH+"/zh_cn/ZQC/theScore.html");
			}
	}
