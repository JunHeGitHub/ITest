/*自定义变量*/
var NewTreenodeId=""; //节点编号(用于-弹出框、数据绑定)
var NewTreerecordType=""; //树分类
/****************************/
...
var SP="sp"
		...
		function load(){
			...
			NewTreerecordType=getQueryString("recordType");
		...
		function getItemTree(jsons){
			...
			function zTreeOnClick(event, treeId, treeNode) {
 	 		parentId=treeNode.id;
   			jsons=treeNode;
   			NewTreenodeId=treeNode.id; //节点赋值
   			window.parent.NewTreenodeIdToUpd=treeNode.id; //节点赋值(改绑)
    		}
          	//添加子节点
          	function addNode_(){
          	        $('#text_').val('');
...
function DigUpdDataToTree(){
var ztree = $.fn.zTree.getZTreeObj("treeDemo");
		var SelData=ztree.getSelectedNodes();
		if(SelData[0]==undefined){
		CheckResult=false;
		window.parent.zinglabs.alert("请选择要绑定到的节点");
		return;
			}
window.parent.closeDig();
window.parent.UpdDataOfTree();
}
function closeDigFn(){
window.parent.closeDig();
}