...
class CMyTree
{
	...
	public CMyRow AppendRow()
	{
		CMyRow row = new CMyRow();
		rows.add(row);
		return row;
	}
...
class CMyTable
{
		  	...
		  	{ 
		  		if( null == row )
		  		{
					if( iRow > m_tree.RowCount() )
						...
						row = (CMyRow)m_tree.rows.get(iRow-1);
		  		}
		  		
		  		CMyCell cell = row.AppendCell();
		  		cell.nRow = iRow;
		  		cell.ID = rs.getInt(1);
	...
	public boolean PrcessTree()
	{
		...
		{
			yzCell = (CMyCell)m_tree.leafcells.get(i);
...
//					System.out.println("getPfx1Pfx2MinMaxValue end++++++++++++++");
					 ...
					 if(scoreList != null && scoreList.size() == leafCells.size()){
	                    	if( bReadOnly ){
	                    		...
	                    		sTable += "</select>";
	                    			...
	                    			sTable += scoreList.get(h) +"<select " + title +"  style='vertical-align:middle;' maxlength='5' name=\"value(" + cell.ID + "_" + h + ")\" value=" +scoreList.get(h) + " >" ;
	                    			for(int n=0;n<rangeList.size();n++){
		                    			sTable +="<option value='"+rangeList.get(n)+"'>"+rangeList.get(n)+"</option>";
		                    		}
		                    		sTable += "</select>";
				...
				sTable += "</tr>";				
				//--					
				//sTable +="<tr align=middle ><td>是否一票否决</td><td>";
...
//					System.out.println("getPfx1Pfx2MinMaxValue end -----------------------");
			...
			//		Iterator iter = map.entrySet().iterator(); 
					...
					Set key = map.keySet();     //ͨ��keySet()���� ȡ�����е�key
	                    	...
	                    	if( bReadOnly ){
	                    		...
	                    		for(int n=0;n<rangeList.size();n++){
	                    			for (int i = 0; i < keys.length; i++) { 
                    					String ke = (String)keys[i];      // ͨ���±�ȡ���������keyֵ,��kȥ����ȡ���keyֵ
			...
			//--
				for(int i=0; i<nRows; i++)
				{
					...
					for(int j=0; j<nCells; j++)
					{
						...
						sTable += "<td   align=middle class='tdcolour1' rowSpan=\"" + cell.nRowSpan + "\" colSpan=\"" + 
						cell.nColSpan + "\" " + sTdProp + " >" + cell.DESCRIPTION + "("+cell.perent+")"+ "</td>";		//"("+cell.perent+"%)"	
					}
					sTable += "</tr>";
				}	
			//--	
...
public class MyTable 
{
	 ...
	 throws SQLException
	{
		...
		{ 
			...
			CMyTable table = new CMyTable(conn,grade_index);
			table.PrcessTree();
...
//				conn.close();
	 ...
	 throws SQLException
	{
		...
		{ 
			...
			for(int h=0; h<leafCells.size(); h++)
			{
				CMyCell cell = (CMyCell)leafCells.get(h);
				Integer iID = new Integer(cell.ID);
				leafList.add(iID);				
			}			
		} 
...
//				conn.close();
	...
	throws SQLException
	{
		...
		{ 
			...
			{
				CMyCell cell = (CMyCell)tmplist.get(i);
				if( cell.isLeaf )
				{
