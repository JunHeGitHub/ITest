...
public class MessageUpdate implements ActivitiFromInterface {
		...
		try {
			...
			String strArray[] = fields.trim().split(",");
				...
				if (formmap.get("flowNode").equals("Over")) {
					//结束修改留言状态为已处理
					Map UpdMessbook = new HashMap();
					UpdMessbook.put("tableName", "Z_Messagebook");
					UpdMessbook.put("ZT", "已处理");
					UpdMessbook.put("id",rmap.get("dataId"));
					//判断结束流程目的是否为回馈信息,做相应数据填充
					if (formmap.get("flow_type").equals("-2")) {
						rmap.put("replyTime", date);
						rmap.put("published", date);
						UpdMessbook.put("id",rmap.get("dataId"));
						UpdMessbook.put("HF",rmap.get("HF"));
					}
					getService().act_update(UpdMessbook);
				}
