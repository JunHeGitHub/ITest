...
/**
 * @author 
 * 1) chuan.li@zinglabs.com 2012-01-31
 * @function 中文转汉语拼音
 * @description 同音字问题，如果出现某些汉字无法转换 
 * 需手动加入对应汉字加到字库和拼音映射表。 
 * 均不影响目前使用
 * 
 */	
 ...
 * @param {} value
 */
function U_trimSpanceAll(value){
 ...
 * @return {}　new Date
 */
function U_BuildDate(dateString,formatter)
{
    var today = new Date();
 ...
 * @return {}	返回天数
 */
function U_DateDiffer(startDate, endDate) {
	//得到时间戳相减 得到以毫秒为单位的差
 ...
 * @return {}　天数
 */
function U_DateMonthDays(year,month){
...
function ajaxFunction(url, params, sync, fn) {
	var rdata = false;
	try{
		$.ajax({
					async : sync,
...
/**
 * 根据一个元素的高度，改变另一个元素的高度
 * @param {} p 
 * @return {String} 返回运行结果
 */
function U_reCssHeight(p){
	var setting={
		//高度源标识的上级或容器
		srcParent:'',
		//高度源标识的上级或容器获取方式[css id]
		srcParentType:'id',
		//高度源标识
		src:'',
		//获取高度源的方式[css id]
	...
	try{
		var src=p["src"];
		var dist=p["dist"];
		var psrc=p["srcParent"];
		var pdist=p["distParent"];
		if(psrc!=''){
			psrc=U_jquery_getElement({
					id:psrc,
					type:p["srcParentType"]
				});
		}
		if(src!=''){
			if(U_object_isNotNull(psrc)){
				src=U_jquery_getElement({
					elParent:psrc,
					id:src,
					type:p["srcType"]
				});
		...
		if(pdist!=''){
			pdist=U_jquery_getElement({
				id:pdist,
				type:p["distParentType"]
			});
		}
		if(dist!=''){
			if(U_object_isNotNull(pdist)){
				dist=U_jquery_getElement({
					elParent:pdist,
					id:dist,
					type:p["distType"]
				});
		...
		if(U_object_isNotNull(src) && U_object_isNotNull(dist)){
			var srcHeight=$(src).css('height');
			//alert(srcHeight);
			var h=parseInt(srcHeight) + parseInt(p["fix"]);
			//alert(h);
			$(dist).css("height",h);
			//alert($(dist).css());
		}else{
			return 'com_reDivHeight not have src or dist.';
		}
	}catch(e){
		return 'com_reDivHeight error.';
	}
	return 'com_reDivHeight success';
}

/**
 * jquery寻找元素的封装，便于一些情况下的特殊反复调用。
 * 可以直接返回元素，也可以在某一父元素下寻找元素。可以通过ID 与CSS来寻找
 * @param {} p
 * @return {String}
 */
function U_jquery_getElement(p){
	var setting={
		//上级容器对象
		elParent:'',
		//获取元素的标识
	...
	var elParent=p["elParent"];
	var id=p["id"];
	var type=p["type"];
	if(type=='css'){
		if(U_object_isNotNull(elParent)){
			return $(elParent).find("." + id);
		}else{
			return $("." + id);
		}
	}else if(type=='id'){
 ...
 * @param {} p
 */
function U_jquery_scrollLeft_moveTo(p){
	var settings={
		//元素对象
		el:'',
		//移动量
		pos:80,
		//移动模式[move ++] move:直接模式，直接到指定位置；++：递增模式，在当前位置加移动量(pos属性)
		mode:'move'
	};
	p=$().extend(settings,p);
	el=p["el"];
	if(U_object_isNotNull(el)){
		var pos=parseInt(p["pos"]);
		var mode=p["mode"];
		if(mode=="move"){
			el.scrollLeft=pos;
		}else if(mode=="++"){
			el.scrollLeft= el.scrollLeft +pos;
		}
	}
}

function U_clientSideInclude(id, url) {
    var req = false;
  ...
  if(search.length>1){
    ...
    for(var i=0; i<argList.length; i++){
      ...
      newArg = argList[i].split('=');
        if(newArg[1]!=undefined&&newArg[1]!=""&&newArg[1]!=null){
            passedArgs[unescape(newArg[0])] = runicode(unescape(newArg[1]));
        }else{
            passedArgs[unescape(newArg[0])] = "";
        }
    }
  }
