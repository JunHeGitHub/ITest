package com.zinglabs.apps.ZhongHangYeMian;
...
public static Logger SKIP_Logger = LoggerFactory.getLogger("ZKM");
	 ...
	 * @throws Exception 
	 */
	public void testAdd(Map map) throws Exception{
		String idStr = DAOTools.getTableId("Z_Management","ZKM",null,null,null);
		map.put("id", Integer.parseInt(idStr));
		getServiceSqlSession().db_insert("zhonghang_insert",map);
	}
	...
	public void testDelete(Map map){
		getServiceSqlSession().db_delete("zhonghang_delete",map);
	}
	/**
	 *  对应的配置
	...
	public void testUpdate(Map map){
		getServiceSqlSession().db_update("zhonghang_update",map);
	}
	...
	public List testSelect(Map map){
		List list=getServiceSqlSession().db_selectList("zhonghang_select",map);
		return list;
	}
	
	public int testSelectOne(Map map){	
		return ((Integer)getServiceSqlSession().db_selectOne("zhonghang_selectOne", map)).intValue();
	}
	public List testSelectAll(){
		List list=getServiceSqlSession().db_selectList("zhonghang_select");
		return list;
	}
	/**
	 * 事物操作 需要数据库支持
	...
	public void testBatchExecute(List<Object>params){
		try {
		  ...
		  * 184	dbip	   128.128.128.128	securityDB数据库IP	    database	 ZKM数据库配置	ZKM	2014-06-16 16:31:19
			...
			*/
			//调用dbid 等于ZDesk的连接
			int results[]=getServiceSqlSession().batchExecute("zhonghang_insert", params, "ZKM");
	...
	public AppSqlSessionDbid getServiceSqlSession() {
		return (AppSqlSessionDbid)this.getBean("zh_SqlSession");
	}
